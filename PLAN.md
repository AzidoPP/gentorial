...
- 下列包均已具备 `package.json`、TypeScript 入口、README、测试、构建脚本和可检查的 npm tarball：
  ...
  - `packages/create`
- `examples/minimal` 已用确定性 mock 打通概念锚点、生成组件、静态回退与 VitePress 构建。
- `pnpm check` 已通过；当前共有 8 个测试文件、19 个测试。
...
切换到本项目后，先完整阅读本文，再从“阶段 1”开始。不要为了快速演示而把 AI、页面引擎或领域校验写进核心包。
...
4. 在无 AI、离线、请求失败或校验失败时保留明确的教学事实与静态回退内容。
...
- 页面首先呈现作者明写的概念锚点。
- 学习者可以填写自己的背景、目标、语言风格或示例偏好。
- 每个生成区块独立请求、取消、重试和恢复。
- 生成结果以受控课程块呈现，不注入任意脚本、HTML 或组件代码。
- 错误信息说明失败发生在哪一层，并允许继续阅读静态内容。
...
概念锚点始终可见。生成区块必须支持静态快照或作者回退内容；AI 不可用时，页面仍能完成基本教学。
...
flowchart LR
  ...
  B --> C["Course Manifest<br/>Concept + Generate AST"]
...
`statement` 保存作者原文。提示编译器可引用它，但任何生成结果都不能改写源内容。
...
运行时只接收统一的 `generate(input)` 函数，不导入任何具体提供方 SDK。
...
职责：注册 Vue 组件、样式和无障碍交互。主题不得负责解析 Markdown 或直接请求模型。
...
1. **解析**：从课程清单取得 `GenerateSpec` 及其引用的 `ConceptSpec`。
2. **编译**：合并课程级准确性策略、概念原文、局部提示、学习者偏好和输出协议。
...
- 要求在 `grounding.conceptIds` 中声明实际依据。
...
- 初始化完成后的默认项目必须无需密钥即可启动，使用静态回退或明确标注的 mock。
...
状态：**进行中（包初始化与最小 mock 链路已完成）**。

目标：从 Markdown 指令到 VitePress 页面完整贯通，但使用确定性的 mock 生成器。

任务：

1. 实现 `@gentorial/core` 的协议、类型和诊断。
2. 实现 `@gentorial/content` 的纯文本解析与 Node 目录编译。
3. 实现 `@gentorial/runtime-vue` 的概念、生成区块和课程块渲染。
4. 实现 `@gentorial/engine-vitepress` 的 Markdown-it 接入。
5. 实现 `@gentorial/theme-default`。
6. 建立 `examples/minimal`，使用两个 `switch` 生成提示。
7. 实现确定性 mock，使生成、校验、错误和回退状态都可演示。
8. 为每层建立测试。
...
1. 实现 `PromptCompiler`、`ProviderAdapter`、`AITransport`。
2. 先完成一个提供方适配器；其他提供方通过相同契约测试。
3. 实现作者构建时生成命令。
4. 实现快照格式、内容哈希与失效策略。
5. 实现远程服务接口协议。
...
阶段 1 的第一条可见成果应是：打开 `examples/minimal`，先看到明确的 `switch` 概念，再点击两个生成区块得到经过协议校验的确定性示例。
